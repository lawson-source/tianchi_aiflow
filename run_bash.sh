...
rm -f ${log_line_num} /root/tianchi_entry/result.csv
...
mysql -e "drop database airflow; CREATE DATABASE airflow CHARACTER SET UTF8mb3 COLLATE utf8_general_ci;" --user=root --password=root 
...
echo "starting push_kafka.py"

/opt/python-occlum/bin/python3.7 push_kafka.py --input_filename=/tcdata/predict.csv --server localhost:9092 --input_topic tianchi_input_${time} --output_topic tianchi_output_${time} > push_kafka.log & 
PUSH_KAFKA_PID=$!
...
/opt/python-occlum/bin/python3.7 listen_kafka.py --output_filename=/root/tianchi_entry/result.csv --server localhost:9092 --input_topic tianchi_input_${time} --output_topic tianchi_output_${time}
