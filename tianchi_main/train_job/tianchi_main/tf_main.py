...
def build_model(batch_size, input_dim, nClasses, hidden_layers,keep_prob, reg_weight, lr):
    ...
    for n in range(len(hidden_layers)):
        tmp_name = 'l_' + str(n)
        layers.append(tf.nn.dropout(tf.nn.relu(tf.matmul(layers[-1], weights[tmp_name])+biases[tmp_name]),keep_prob[n]))
...
def train(train_path, model_dir, save_name):
    hidden_layers=[32,64,128,256,128,64,32,16]
    keep_prob=[0.7,0.7,0.5,0.5,0.5,0.7,0.7,0.7]
    nClasses=2
    reg_weight=0.01
    num_epochs=100
    input_dim=75
